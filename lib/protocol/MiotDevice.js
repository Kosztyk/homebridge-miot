...
class MiotDevice extends EventEmitter {
  ...
  // micloud
  shouldUseMiCloud() {
    return this.requiresMiCloud() || this._getMiCloudForce() === true || (this._modelShouldUseMiCloud() && this._canUseMiCloud());
  }

  _modelShouldUseMiCloud() {
    // Do not auto-force MiCloud for yeelink.light.strip2.
    // This model worked locally in the original plugin, and forcing cloud caused repeated login attempts
    // and unresponsive HomeKit controls when the cloud session was not established.
    const cloudPreferredModels = [];
    return cloudPreferredModels.includes(this.getModel());
  }
  ...
  async _connectToLocalDevice() {
    if (!this.isConnectedToLocalDevice()) {
      ...
      this.logger.info(`Device found! Setting up Xiaomi Mi device from local connection!`);
    ...
    if (!this.isConnectedToCloud()) {
      if (this._canUseMiCloud()) {
        ...
        try {
          ...
          this.logger.info(`Successfully connected to MiCloud! Setting up Xiaomi Mi device from MiCloud connection!`);
