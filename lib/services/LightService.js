...
class LightService extends AbstractService {
  ...
  supportsYeelightLanControl() {
    ...
    // If LAN control was recently refused/timed out, temporarily fall back to the
    // normal Xiaomi Mi path. This prevents Homebridge read handlers from repeatedly
    // throwing ECONNREFUSED when Yeelight LAN Control/Developer Mode is disabled
    // or the strip temporarily closes port 55443.
  ...
  handleYeelightLanError(err, context = 'operation') {
    ...
    const isConnectivityError = /ECONNREFUSED|ETIMEDOUT|EHOSTUNREACH|ENETUNREACH|timed out|socket closed/i.test(msg);
    ...
    const now = Date.now();
    if (this.lastYeelightLanErrorKey !== lastKey || !this.lastYeelightLanErrorLog || now - this.lastYeelightLanErrorLog > 60000) {
      this.lastYeelightLanErrorKey = lastKey;
      this.lastYeelightLanErrorLog = now;
      this.getLogger().warn(`[LightService] Yeelight LAN ${context} failed for ${deviceName}: ${msg}. Falling back to cached/Xiaomi Mi state for ${cooldown}s. Check LAN Control/Developer Mode and TCP port 55443.`);
    } else {
      this.getLogger().debug(`[LightService] Yeelight LAN ${context} failed for ${deviceName}: ${msg}`);
    }
  }
