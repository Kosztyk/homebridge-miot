...
class LightService extends AbstractService {
  ...
  supportsYeelightLanControl() {
    ...
    // normal MIOT path. This prevents Homebridge read handlers from repeatedly
  ...
  handleYeelightLanError(err, context = 'operation') {
    ...
    if (this.lastYeelightLanErrorKey !== lastKey || !this.lastYeelightLanErrorLog || now - this.lastYeelightLanErrorLog > 60000) {
      ...
      this.getLogger().warn(`[LightService] Yeelight LAN ${context} failed for ${deviceName}: ${msg}. Falling back to cached/MIOT state for ${cooldown}s. Check LAN Control/Developer Mode and TCP port 55443.`);
    } else {
