...
class LightService extends AbstractService {
  ...
  handleYeelightLanError(err, context = 'operation') {
    ...
    if (this.lastYeelightLanErrorKey !== lastKey || !this.lastYeelightLanErrorLog || now - this.lastYeelightLanErrorLog > 60000) {
      ...
      this.getLogger().warn(`[LightService] Yeelight LAN ${context} failed for ${deviceName}: ${msg}. Falling back to cached/Xiaomi state for ${cooldown}s. Check LAN Control/Developer Mode and TCP port 55443.`);
    } else {
