...
class LightService extends AbstractService {
  ...
  async isLightOn() {
    if (this.isMiotDeviceConnected() || this.supportsYeelightLanControl()) {
      this.scheduleYeelightLanStateRefreshIfNeeded();
      return this.isOn();
    }
    return false;
  }
  ...
  scheduleYeelightLanStateRefreshIfNeeded() {
    // HomeKit read handlers must return immediately. Do not await network I/O here.
    // Background Yeelight polling already keeps values fresh; this is only a
    // best-effort async refresh trigger when HomeKit asks for a current value.
    if (!this.supportsYeelightLanControl()) return;
    if (this.yeelightReadRefreshScheduled) return;

    this.yeelightReadRefreshScheduled = true;
    setTimeout(() => {
      this.yeelightReadRefreshScheduled = false;
      this.safeRefreshYeelightLanStateIfNeeded().catch(() => {});
    }, 0);
  }

  async safeRefreshYeelightLanStateIfNeeded() {
    try {
      await this.refreshYeelightLanStateIfNeeded();
      return true;
    } catch (err) {
      this.handleYeelightLanError(err, 'state refresh');
      return false;
    }
  }

  handleYeelightLanError(err, context = 'operation') {
    const msg = err && err.message ? err.message : String(err);
    const deviceName = this.getDevice().getName();
    const miotDevice = this.getDevice().getMiotDevice && this.getDevice().getMiotDevice();
    const config = (miotDevice && miotDevice.userConfig) || {};
    const cooldown = Math.max(30, Number(config.yeelightLanErrorCooldown) || 300);
    const isConnectivityError = /ECONNREFUSED|ETIMEDOUT|EHOSTUNREACH|ENETUNREACH|EAI_AGAIN|ENOTFOUND|timed out|socket closed/i.test(msg);

    if (isConnectivityError) {
      this.yeelightLanDisabledUntil = Date.now() + (cooldown * 1000);
    }

    const logMessage = `[LightService] Yeelight LAN ${context} failed for ${deviceName}: ${msg}. Falling back to cached/Xiaomi state for ${cooldown}s.`;
    const lastKey = `${context}:${msg}`;
    const now = Date.now();
    const shouldLog = this.lastYeelightLanErrorKey !== lastKey || !this.lastYeelightLanErrorLog || now - this.lastYeelightLanErrorLog > 300000;
    this.lastYeelightLanErrorKey = lastKey;
    this.lastYeelightLanErrorLog = now;

    // By default, keep these connectivity failures out of normal Homebridge logs.
    // If a strip changes IP, is offline, or does not expose TCP/55443, Yeelight LAN
    // state sync will fail repeatedly even though the Xiaomi/MIOT fallback can still
    // keep the accessory usable. Set yeelightLanErrorLogLevel to "warn" while
    // troubleshooting, "debug" to keep it visible only in debug logs, or "off" to
    // suppress it entirely.
    const level = String(config.yeelightLanErrorLogLevel || 'debug').toLowerCase();
    if (level === 'off' || level === 'silent' || level === 'false') return;
    if (!shouldLog && level !== 'debug') return;

    if (level === 'warn' || level === 'warning') {
      this.getLogger().warn(`${logMessage} Check LAN Control/Developer Mode, current IP, and TCP port 55443.`);
    } else {
