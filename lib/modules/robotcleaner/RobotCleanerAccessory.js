...
class RobotCleanerAccessory extends BaseAccessory {
  ...
  initAccessoryObject() {
    ...
    // Keep robot cleaners on the vacuum/robot-cleaner layout by default. HomeKit classic HAP
    // has no native robot vacuum service; the optional Fanv2 experiment is disabled by default
    // so devices like Jamal are not presented as fans or air purifiers in Apple Home.
    this.vacuumRefinedHomeKitView = this.getConfigValue('vacuumRefinedHomeKitView', false);
    this.vacuumLegacySwitch = this.getConfigValue('vacuumLegacySwitch', true);
  ...
  initAccessories(name, uuid) {
    // Keep the accessory in the legacy robot-cleaner/vacuum presentation by default.
    // Only use Fan category if the user explicitly enables vacuumRefinedHomeKitView.
